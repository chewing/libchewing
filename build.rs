fn main() {
    #[cfg(feature = "capi")]
    {
        ...
        if let Ok(cmake_build_dir) = env::var("CMAKE_BINARY_DIR") {
            ...
            #[cfg(target_os = "linux")]
            capi::write_version_script(&mut symbols_file).expect("writing to file");
            #[cfg(target_os = "macos")]
            capi::write_exported_symbols(&mut symbols_file).expect("writing to file");
            #[cfg(target_os = "windows")]
            capi::write_def(&mut symbols_file).expect("writing to file");
        }
    }
}
...
mod capi {
    ...
    #[cfg(target_os = "windows")]
    pub(super) fn write_def(f: &mut impl Write) -> Result<()> {
        writeln!(f, "EXPORTS")?;
    ...
    #[cfg(target_os = "macos")]
    pub(super) fn write_exported_symbols(f: &mut impl Write) -> Result<()> {
        for (version, symbol_list) in SYMBOLS {
            for sym in symbol_list.iter() {
                writeln!(f, "_{sym}")?;
            }
