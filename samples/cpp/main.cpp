...
int main() {
    ...
    csCtx.callbacks.candidate_info =
        [](int pageSize, int numPages, int candidateOnPage, int totalChoices, const char **candidates) {
            std::cout << "Candidates [" << candidateOnPage << "/" << totalChoices << "]\n";
            std::vector<std::string_view> options(candidates, candidates + totalChoices);
            for (int i = 0; i < totalChoices; ++i) {
                std::cout << "  " << i << ": " << options[i] << "\n";
            }
        };
    csCtx.callbacks.buffer =
        [](const char *buf) { std::cout << "Buffer:   " << buf << "\n"; };
    csCtx.callbacks.bopomofo =
        [](const char *buf) { std::cout << "Preedit:  " << buf << "\n"; };
    csCtx.callbacks.commit =
        [](const char *buf) { std::cout << "Commit:   " << buf << "\n"; };
    csCtx.callbacks.logger =
    ...
    while ((ch = std::getchar()) != escapeKey) {
        if (ch == '`') { cs_select_candidate(5); continue; }
        std::cout << "\n---------------------------\n";
